...
struct TouringTabView: View {
    ...
    var body: some View {
        ...
        .onChange(of: isFilteredToZero) { _, isZero in
            if isZero {
    ...
    private var filteredToZeroState: some View {
        centeredState {
            ...
            Button("Clear filters") { model.filter.reset() }
    ...
    /// True when an active filter has narrowed a non-empty window to zero — the
    /// signal for the filtered-to-zero analytics event.
    private var isFilteredToZero: Bool {
        model.phase == .loaded && !model.allConcerts.isEmpty && model.filter.isActive && model.filtered.isEmpty
    }
