...
//  WXYC
//
//  UI smoke for the R1 On Tour tab (#490): the third tab is reachable, its Filter
//  button opens the filter sheet, and a For You card can be dismissed via its
//  overflow menu. Tabs are selected by accessibility identifier (`tab.onTour`) via
//  a normalized-coordinate tap, since the iOS 26 floating tab bar isn't exposed as
//  `XCUIElement.tabBars` and reports items as existing-but-not-hittable (same
//  constraints as `RootTabBarUITests`).
//
...
struct OnTourTabUITests {
    ...
    init() {
        // Reset the dismissed-shows set and force the For You loved seed on, so the
        // dismiss test sees a deterministic shelf regardless of simulator-persisted
        // likes; harmless for the other cases.
    ...
    func dismissForYouCardViaOverflow() async throws {
        ...
        // `-uiTestResetForYou` forces the loved seed on, so given a non-empty live
        // concert window at least one For You card renders. Environment-gated like
        // the rest of this suite (WXYC_SKIP_UI excludes it from CI).
