...
struct RootTabBarUITests {
    ...
    @Test("Tapping Liked reaches the liked-songs surface")
    func likedTabReachable() async throws {
        try await waitForLaunch()
        try await tapTab(likedTab)
        // Assert on the tab's content-surface identifier rather than the empty
        // state: a simulator that has liked songs in a previous run persists
        // them, so the empty-state copy is not a stable launch invariant.
        let content = app.descendants(matching: .any)
            .matching(identifier: "likedTabView")
            .firstMatch
        try await waitUntil(content, is: .exists, timeout: contentTimeout)
    }
