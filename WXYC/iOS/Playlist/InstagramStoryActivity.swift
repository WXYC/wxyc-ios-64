...
final nonisolated class InstagramStoryActivity: UIActivity {

    // Match UIActivity's nonisolated init
    override init() {
        super.init()
    }
    ...
    override func canPerform(withActivityItems activityItems: [Any]) -> Bool {
        ...
        // UIApplication APIs must be called on main; hop to main synchronously
    ...
    override func perform() {
        ...
        if let instagramURL = URL(string: "instagram-stories://share") {
            let result = sync { @MainActor in
                await UIApplication.shared.open(instagramURL, options: [:])
            }
    ...
    // Synchronously bridge to @MainActor work from a nonisolated context.
