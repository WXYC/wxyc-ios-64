...
/// Main audio streaming player using MiniMP3 for decoding
/// Coordinates HTTP streaming, MP3 decoding, and audio playback
@MainActor
@Observable
public final class MiniMP3Streamer {
    ...
    public private(set) var state: StreamingAudioState = .idle {
        didSet {
            if state != oldValue {
                delegate?.miniMP3Streamer(didChangeState: state)
            }
        }
    }
    ...
    public func play() async throws {
        ...
        do {
            ...
            state = .error(error)
            delegate?.miniMP3Streamer(didEncounterError: error)
            throw error
        }
    }
...
private final class HTTPStreamClientDelegateAdapter: HTTPStreamClientDelegate, @unchecked Sendable {
    weak var streamer: MiniMP3Streamer?
