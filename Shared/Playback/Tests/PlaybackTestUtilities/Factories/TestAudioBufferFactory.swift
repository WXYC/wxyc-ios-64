...
public enum TestAudioBufferFactory {
    /// Standard format used by AudioEnginePlayer: 44.1kHz stereo float32
    public static func makeStandardFormat() -> AVAudioFormat {
        AVAudioFormat(
    ...
    public static func makeSilentBuffer(frameCount: AVAudioFrameCount = 4096) -> AVAudioPCMBuffer {
    ...
    public static func makeTestToneBuffer(frameCount: AVAudioFrameCount = 4096, frequency: Float = 440.0) -> AVAudioPCMBuffer {
        let format = makeStandardFormat()
        let buffer = AVAudioPCMBuffer(pcmFormat: format, frameCapacity: frameCount)!
        buffer.frameLength = frameCount
    ...
    public static func loadMP3TestData() throws -> Data {
...
public enum TestAudioError: Error {
