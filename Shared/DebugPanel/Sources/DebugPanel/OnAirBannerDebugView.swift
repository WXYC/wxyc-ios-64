...
public struct OnAirBannerDebugView: View {
        ...
        NavigationStack {
            Form {
                ...
                DisclosureGroup("Indicator", isExpanded: $indicatorExpanded) {
                    ...
                    LabeledSlider("Blur Radius", value: $state.indicatorBlurRadius, in: 0...30, format: Format.oneDecimal)
                }

                DisclosureGroup("DJ Handle", isExpanded: $handleExpanded) {
                    LabeledSlider(SFProFontAxis.weight.displayName, value: $state.handleWeight, in: SFProFontAxis.weight.range, format: Format.noDecimals)
                    LabeledSlider(SFProFontAxis.width.displayName, value: $state.handleWidth, in: SFProFontAxis.width.range, format: Format.noDecimals)
                    LabeledSlider(SFProFontAxis.opticalSize.displayName, value: $state.handleOpticalSize, in: SFProFontAxis.opticalSize.range, format: Format.noDecimals)
                    LabeledSlider(SFProFontAxis.grade.displayName, value: $state.handleGrade, in: SFProFontAxis.grade.range, format: Format.noDecimals)
                }

                DisclosureGroup("Adaptive Width", isExpanded: $adaptiveExpanded) {
                    Toggle("Condense to fit", isOn: $state.adaptiveWidth)
                    LabeledSlider("Width Floor", value: $state.handleWidthFloor, in: SFProFontAxis.width.range, format: Format.noDecimals)
                ...
                DisclosureGroup("Handle Wave", isExpanded: $waveExpanded) {
                    ...
                    LabeledSlider("Depth", value: $state.waveDepth, in: 0...536, format: Format.noDecimals)
                    LabeledSlider("Thinning", value: $state.waveWeightDepth, in: 0...647, format: Format.noDecimals)
                    LabeledSlider("Crest Width", value: $state.waveCrestHalfWidth, in: 0.1...1.0)
                    LabeledSlider("Repetitions", value: $state.waveRepetitions, in: 1...5, step: 1, format: Format.noDecimals)
                ...
                DisclosureGroup("Spacing", isExpanded: $spacingExpanded) {
                    LabeledSlider("Below ON AIR", value: $state.onAirSpacing, in: 0...40, format: Format.oneDecimal)
                    LabeledSlider("Handle Line Spacing", value: $state.handleLineSpacing, in: 0...30, format: Format.oneDecimal)
                }
            }
    ...
    /// `static` rather than instance properties: as stored `let`s they would be
    /// two closure boxes allocated per view value, on a `View` whose body
    /// re-evaluates on every drag tick of seventeen sliders. They capture
    /// nothing, so the call sites read the same either way.
    private enum Format {
        ...
        static let oneDecimal: @Sendable (Double) -> String = { String(format: "%.1f", $0) }

        /// Whole numbers (e.g. SF Pro axis values, wave depth/repetitions).
        static let noDecimals: @Sendable (Double) -> String = { String(format: "%.0f", $0) }
    }
}
#endif
