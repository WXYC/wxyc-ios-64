...
//  byte-identical refresh is deduped, and a status change on a still-present
//  concert (OT-C5's axis) re-reconciles. Exercised over a recording reconciler
//  spy so the assertions are on exactly which windows/inputs reached
//  `reconcile`, independent of CoreSpotlight.
//
...
struct ConcertSpotlightWindowObserverTests {
    ...
    @Test("dedups a byte-identical refresh")
    func dedupsIdenticalRefresh() async {
        let reconciler = RecordingReconciler()
        let observer = ConcertSpotlightWindowObserver()
        ...
        // Same ids, same statuses — a pull-to-refresh that returned the same data.
