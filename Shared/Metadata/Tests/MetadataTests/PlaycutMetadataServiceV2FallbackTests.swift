...
//  fields in favor of a single `Core.WXYCProxyClient`. This file injects a
//  stub-`URLProtocol`-backed double via the service's `urlSession:`
//  parameter, same as `PlaycutMetadataServiceCachingTests.swift`.
//
//  #786: that double used to be a private `MetadataV2MockWebSession` type
//  hand-rolled in this file, forked from `PlaycutMetadataServiceCachingTests
//  .swift`'s `MetadataMockWebSession` only because each needed its own
//  static `URLProtocol`-registration state. Both are gone — these tests now
//  construct `CoreTesting.PatternRoutingWebSession` directly, and this file
//  joins `PlaycutMetadataServiceHTTPTests` as an `extension` rather than its
//  own `@Suite`: `PatternRoutingWebSession` delegates to
//  `CoreTesting.QueuedStubURLProtocol`'s single registered class, so it
//  inherits — rather than needs its own workaround for — that type's
//  one-adopter-per-bundle constraint (see both types' header docs, and
//  `DiscogsAPIEntityResolverCachingTests.swift` /
//  `PlaycutMetadataResolverTests.swift` for the same arrangement already
//  established elsewhere in this package). `.serialized` is inherited from
//  `PlaycutMetadataServiceHTTPTests`'s own `@Suite` declaration.
//
