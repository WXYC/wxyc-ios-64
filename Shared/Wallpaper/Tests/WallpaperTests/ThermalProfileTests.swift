...
struct ThermalProfileTests {
    ...
    func updateIncrementsSampleCount() {
        ...
        profile.update(fps: 55, scale: 0.9, lod: 0.8)
        #expect(profile.sampleCount == 1)

        profile.update(fps: 50, scale: 0.8, lod: 0.7)
        #expect(profile.sampleCount == 2)
    }

    @Test("isAtMaxQuality returns correct value")
    func isAtMaxQuality() {
        let maxProfile = ThermalProfile(shaderId: "test", fps: 60, scale: 1.0, lod: 1.0)
        #expect(maxProfile.isAtMaxQuality)

        let throttledProfile = ThermalProfile(shaderId: "test", fps: 45, scale: 0.8, lod: 0.5)
        #expect(!throttledProfile.isAtMaxQuality)

        // Also throttled if only LOD is reduced
        let lodThrottledProfile = ThermalProfile(shaderId: "test", fps: 60, scale: 1.0, lod: 0.5)
    ...
    func markStabilized() {
        var profile = ThermalProfile(shaderId: "test")
        profile.update(fps: 55, scale: 0.9, lod: 0.8)
        profile.update(fps: 50, scale: 0.85, lod: 0.7)
        profile.update(fps: 50, scale: 0.85, lod: 0.7)
