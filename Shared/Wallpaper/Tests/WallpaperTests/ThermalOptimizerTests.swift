...
struct ThermalOptimizerTests {
    ...
    func noAdjustmentInDeadZone() {
        let profile = ThermalProfile(shaderId: "test", fps: 50, scale: 0.8, lod: 0.7)
    ...
    func heatingReducesQuality() {
        let profile = ThermalProfile(shaderId: "test", fps: 60, scale: 1.0, lod: 1.0)

        let result = optimizer.optimize(current: profile, momentum: 0.5)

        // LOD is reduced first, then scale, then FPS
        #expect(result.lod < profile.lod)
        #expect(result.scale < profile.scale)
        #expect(result.fps < profile.fps)
    }
    ...
    func coolingRestoresQuality() {
        let profile = ThermalProfile(shaderId: "test", fps: 30, scale: 0.6, lod: 0.5)
    ...
    func scalePreferredOnHeating() {
        ...
        // Scale should be reduced proportionally more (60% weight vs 20% for FPS)
    ...
    func resultsClamped() {
        // Test lower bounds
        let lowProfile = ThermalProfile(shaderId: "test", fps: 16, scale: 0.51, lod: 0.1)
        ...
        // Test upper bounds
        let highProfile = ThermalProfile(shaderId: "test", fps: 59, scale: 0.99, lod: 0.95)
    ...
    func recoverySlower() {
        let profile = ThermalProfile(shaderId: "test", fps: 45, scale: 0.75, lod: 0.6)
