...
//  Wallpaper
//
//  Low-poly refractive pool tiles effect with single-octave linear-interpolated noise.
//  Creates a faceted, geometric water surface aesthetic.
//
//  Based on PoolSuite.metal
...
// === Noise (3D from 2D noise texture) with LINEAR interpolation for low-poly look ===
...
// Linear interpolation version - creates faceted/low-poly appearance
static inline float noise3D_linear(float3 x, texture2d<float> noiseTex, sampler s) {
    float3 p = floor(x);
    float3 f = fract(x);  // Linear interpolation - NO smoothstep!
...
// Smoothstep version for subtle variations (tiles, caustics)
static inline float noise3D_smooth(float3 x, texture2d<float> noiseTex, sampler s) {
    float3 p = floor(x);
    float3 f = x - p;
    ...
    float a = noiseTex.sample(s, (uv + 0.5f) * INV_256).x;
    float b = noiseTex.sample(s, (uv + NOISE_OFFSET2) * INV_256).x;
    return mix(b, a, f.z) * 2.0f - 1.0f;
}

// Single-octave noise for low-poly surface displacement
static inline float getNoise_lowpoly(float3 p, float time, float noiseStrength, float noiseScale, texture2d<float> noiseTex, sampler s) {
    // Single octave with configurable scale - larger scale = larger facets
    return noiseStrength * noise3D_linear((p + 0.3f * time) * noiseScale, noiseTex, s);
}
...
// === Distance Ops / Scene ===
static inline float2 opU(float2 a, float2 b) { return (a.x < b.x) ? a : b; }
...
static inline float2 mapScene(
    ...
    // Shape 0: cube (low-poly noisy surface)
    float dCube = sdBox(pos, float3(cubeHalfSize));
    dCube += getNoise_lowpoly(pos, time, noiseStrength, noiseScale, noiseTex, s);
...
static inline float sdScene(float3 pos, float cubeHalfSize, float floorZ, float time, float noiseStrength, float noiseScale, texture2d<float> noiseTex, sampler s) {
    return mapScene(pos, cubeHalfSize, floorZ, time, noiseStrength, noiseScale, noiseTex, s).x;
}

static inline float3 calculateNormal(float3 p, float cubeHalfSize, float floorZ, float time, float noiseStrength, float noiseScale, texture2d<float> noiseTex, sampler s) {
    const float eps = 0.01f;
    float3 ex = float3(eps, 0.0f, 0.0f);
    float3 ey = float3(0.0f, eps, 0.0f);
    float3 ez = float3(0.0f, 0.0f, eps);

    float gradX = sdScene(p + ex, cubeHalfSize, floorZ, time, noiseStrength, noiseScale, noiseTex, s) - sdScene(p - ex, cubeHalfSize, floorZ, time, noiseStrength, noiseScale, noiseTex, s);
    float gradY = sdScene(p + ey, cubeHalfSize, floorZ, time, noiseStrength, noiseScale, noiseTex, s) - sdScene(p - ey, cubeHalfSize, floorZ, time, noiseStrength, noiseScale, noiseTex, s);
    float gradZ = sdScene(p + ez, cubeHalfSize, floorZ, time, noiseStrength, noiseScale, noiseTex, s) - sdScene(p - ez, cubeHalfSize, floorZ, time, noiseStrength, noiseScale, noiseTex, s);

    return normalize(float3(gradX, gradY, gradZ));
}

// Returns (distance, shapeIndex)
static inline float2 rayMarch(float3 rayOri, float3 rayDir, float cubeHalfSize, float floorZ, float time, float noiseStrength, float noiseScale, int maxSteps, texture2d<float> noiseTex, sampler s) {
    ...
    for (int i = 0; i < maxSteps; ++i) {
        float2 res = mapScene(rayOri + totalDistance * rayDir, cubeHalfSize, floorZ, time, noiseStrength, noiseScale, noiseTex, s);
...
// === Pool floor (procedural pink tiles + white grout) ===
...
static inline float3 poolFloorBaseColor(float3 worldPos, float time, float3 tileColor, texture2d<float> noiseTex, sampler s) {
    ...
    float v = noise3D_smooth(float3(tileID * 0.25f, 0.0f) + float3(0.0f, 0.0f, time * 0.03f), noiseTex, s);
    float3 variedTileColor = tileColor * (0.92f + 0.08f * v);

    float3 base = mix(variedTileColor, groutColor, g);

    // Subtle caustic-ish modulation
    float c = 0.5f + 0.5f * noise3D_smooth(float3(uv * 0.8f, time * 0.1f), noiseTex, s);
...
static inline float3 skyColor(float3 dir, float time, texture2d<float> noiseTex, sampler s) {
    ...
    // Soft pink/coral sky gradient for Box Suite
    float3 horizon = float3(1.0f, 0.85f, 0.88f);
    float3 zenith  = float3(0.22f, 0.12f, 0.18f);
    float3 col = mix(horizon, zenith, powr(t, 1.2f));

    float n = noise3D_smooth(d * 4.0f + float3(0.0f, 0.0f, time * 0.05f), noiseTex, s);
    col += 0.04f * n;
...
static inline float3 render(float3 rayOri, float3 rayDir, float cubeHalfSize, float floorZ, float time,
                            ...
                            float noiseScale, int maxSteps, texture2d<float> noiseTex, sampler s) {
    time /= 2.0;

    float3 color = powr(skyColor(rayDir, time, noiseTex, s), float3(2.2f));

    float2 res = rayMarch(rayOri, rayDir, cubeHalfSize, floorZ, time, noiseStrength, noiseScale, maxSteps, noiseTex, s);
    ...
    if (shapeIndex >= 0) {
        ...
        if (shapeIndex == 1) {
            ...
            // Cube: refractive
            float3 normal = calculateNormal(pos, cubeHalfSize, floorZ, time, noiseStrength, noiseScale, noiseTex, s);
...
fragment half4 boxSuiteFrag(
    ...
    // LOD 1.0 = 64 steps, LOD 0.0 = 16 steps
    ...
    // Hardcoded noiseScale (renderer supports max 8 parameters)
    ...
    float uvMax = max(0.5f, 0.5f * aspect);
    ...
    for (int k = 0; k < AA; ++k) {
        ...
        totalColor += render(rayOri, rayDir, cubeHalfSize, floorZ, u.time,
                             ...
                             noiseScale, maxSteps, noiseTex, s);
    }
