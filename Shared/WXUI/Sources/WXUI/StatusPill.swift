...
//  WXUI
//
//  A filled, monospaced status chip: one palette table shared by every "state"
//  tag in the app (On Tour feed rows, the Box Office ticket, the concert poster
//  hero) instead of four independently hand-maintained (fill, ink) switches.
//  Canon mechanics — padding, font, kerning — are fixed; only the color pair
//  varies per ``Style``, and only via the shared table unless a caller has a
//  genuine reason (a theme-derived accent, say) to supply its own via
//  `paletteOverride`.
//
//  Chips have no outline. There is deliberately no border in the palette and no
//  stroke overlay in the body: every surface had settled on a clear border, so
//  the machinery was removed rather than left as a slot every caller has to
//  remember to pass `.clear` for.
//
...
//  original colors back. What they still share from here is the *mechanics*:
//  padding, font, kerning. The canon table below remains the default for
//  anything adopting this type fresh.
//
...
/// A small filled capsule showing an uppercased status word — "ON SALE",
/// "SOLD OUT", "FREE", and so on.
///
/// The color pair for each ``Style`` lives in one canon table
...
/// `paletteOverride` to replace the resolved pair for that one instance. The
/// override replaces colors only: padding, font, and kerning stay canon
/// everywhere.
public struct StatusPill: View {
    ...
    let text: String
    let style: Style
    let paletteOverride: (fill: Color, ink: Color)?
    ...
    ///   - paletteOverride: An explicit `(fill, ink)` pair to use
    ...
    public init(
        text: String,
        style: Style,
        paletteOverride: (fill: Color, ink: Color)? = nil
    ) {
        self.text = text
        self.style = style
    ...
    /// The color pair to render: `override` when supplied, otherwise the
    ...
    ///
    /// **Every style is a solid fill**, the way "on sale" has always read.
    /// Differentiation is carried by hue alone, never by fill weight. The
    /// earlier table made only `.prominent` solid and rendered the
    ...
    public static func palette(for style: Style) -> (fill: Color, ink: Color) {
        switch style {
        case .prominent:
