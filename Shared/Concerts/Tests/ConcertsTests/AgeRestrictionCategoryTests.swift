...
struct AgeRestrictionCategoryTests {
    ...
    static let cases: [(String?, AgeRestrictionCategory)] = [
        ...
        // "0+" / "0 and up" means *no* minimum age — that's all-ages, not a
        // restriction. Reading it as `.restricted(minAge: 0)` would wrongly hide
        // an effectively all-ages show behind the All-ages filter.
        ("0+", .allAges),
        ("0 and up", .allAges),
