...
| **Core** | Core types (RadioStation, Playcut, etc.); the `FileStorage`/`AppSupportFileStorage` durable never-evict byte-file seam (`CoreTesting` for the `InMemoryFileStorage` double), shared by LikedSongs and Concerts; dependency-free concurrency utilities (`SerialHandoff` for ordered handoff out of a synchronous MainActor caller, `ForegroundVisibility` for reading a `ScenePhase`, alongside `Cancellation`/`TimedOperation`/`ExponentialBackoffTimer`) |
