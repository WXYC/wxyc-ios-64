...
SPM-runnable packages (host-tested via `swift test`): AnalyticsMacros, Core, Caching, Analytics, ColorPalette, Playlist, Artwork, Metadata. The remainder fall through to xcodebuild: Logger (shared-state race in `Logger.addDestination` across parallel test suites), Playback (MP3Streamer state-tracking diverges between macOS host AudioToolbox and the iOS simulator), AppServices (`MockURLProtocol` static-handler race), PartyHorn (Bundle.module), MusicShareKit (SwiftUI `#Preview`), PlayerHeaderView / Wallpaper (submodule).
