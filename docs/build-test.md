...
The load-bearing input above is the *annotation set on our own declarations*, not the package's macOS floor: it is `@available(iOS 27.0, *)` with no macOS platform, so on a host build there is no macOS version above which the compiler will accept the reference. Widening those annotations to include macOS is therefore the fix, and it removes the host-build failure this section describes — so treat this paragraph as describing the state of `Shared/Intents` at the time of writing (2026-08-11), and re-check `git grep '@available(iOS 27.0' Shared/Intents/Sources` before relying on it. If the annotations read `@available(iOS 27.0, macOS 27.0, …)`, this failure mode is gone and only the `#if compiler(>=6.4)` half of this section still applies.
