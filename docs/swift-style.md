...
- Refer to Logger's category type as `LogCategory` (the alias `Logger.swift` defines for `Category`), not the bare `Category`. Files that need both `Logger` and `PostHog`/`Sentry` in scope hit an ambiguous-name collision on the bare name; `LogCategory` sidesteps it with a plain `import Logger` instead of a scoped `import struct Logger.Category` workaround.
- A `.shared` singleton facade with no mutable stored state — no `var`, no lock-guarded storage, nothing lazily cached — should be `struct: Sendable`, not `final class @unchecked Sendable`. The compiler verifies a stateless struct's `Sendable` conformance for free; `@unchecked` on a class with zero mutable state is paying for a thread-safety story that doesn't exist. Reach for `@unchecked Sendable` only when there is real mutable state to protect (an `NSLock`/`Mutex`-guarded var, a lazily-initialized cache) or the type must keep class (`AnyObject`) identity for a reason like `NotificationCenter` subject filtering.
