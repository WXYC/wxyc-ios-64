...
- **Analytics identity**: like/shelf events carry no artist ids (strictest reading of the privacy invariant). Loosening to artist-level product analytics is a deliberate future opt-in.
  ...
  - **Amended 2026-08-31:** the opt-in was extended to *intent*. The line now sits between browse and intent rather than between likes and everything else: **what we showed you stays anonymous, what you chose to act on does not.** Shelf and filter events are unchanged and still assert counts-only in their tests. The identity payload is one type, `ConcertIdentity` (Analytics), composed by every intent event; `artist`/`artist_id` deliberately reuse `song_like_toggled`'s key names so a liked-artist cohort joins against intent without aliasing, and `Concert.analyticsIdentity` (app target) is the single mapping from a show to those keys. Events so far:
    - `concert_detail_viewed` — every opening of a concert detail, tagged with the arrival path (`source`: `row` / `for_you` / `deep_link`). This is the denominator for actions taken *on the detail*: without it a ticket tap had no rate, because `for_you_card_tapped` covered only the shelf path. **Filter the numerator to `surface = 'detail'`** — the action events also fire from the row context menu and the playcut keepsake ticket, neither of which opens a detail, so an unfiltered rate counts three surfaces against one.
    - `concert_tickets_tapped` — the outbound box-office CTA, from the concert detail, the playcut keepsake, or the row's context menu (`surface`).
    - `concert_directions_tapped` — the "Where" card's chip, that card's map, or the row's context menu. Its own name because for a free show the CTA reads "RSVP" and directions is the truer attendance signal.
    - `concert_calendar_flow` — replaces `concert_calendar_added` (which had never fired) with `outcome`: `requested` / `denied` / `cancelled` / `saved` / `failed`. Absorbs the Siri path as `surface: "siri"`.
    - `concert_share_initiated` — moved up from the browse tier. Choosing to send a specific show to a friend is the strongest intent signal on the tab.
  - **`concert_deep_link_opened` deliberately stays anonymous**, and not under the browse rule: the *recipient* didn't choose that band, a friend did, so attributing it as their taste would poison the affinity data the tier exists to collect. The band arrives a millisecond later on `concert_detail_viewed` with honest semantics. Its `source` gained a `webBanner` case so arrivals from the show's own web page stop being counted as Spotlight and Shortcuts; the app reads `?src=web` off the banner's `app-argument`, so the corresponding `wxyc-links-registry` change can ship afterwards.
