...
7. **No new reason for tile taps — the signal already exists.** `PlaybackReason.siriIntent` has exactly one production call site in the repo: `AppLifecycleModifier.swift:145`, the `INPlayMediaIntent`-continuation branch itself. Voice requests land on `.playIntent` / `.playAudioSchemaIntent` instead, and `PlaybackStartedEvent` already captures `reason.rawValue` (`AudioPlayerController.swift:648`). Adding a reason here would orphan `.siriIntent` while it is still asserted at `PlaybackSourceTests.swift:47,91`.

   **Correction, from PR 2's code review:** the sentence this step originally carried — "`"Siri intent"` in PostHog *is* the tile/continuation series today" — is **false**. That one call site is unreachable (see #830 and the Observation section), so the series is structurally empty. The conclusion of this step survives anyway, for a different reason: PR 1 should still not add a reason, because PR 2 adds `.mediaSuggestion` and *that* is the tile series. But do not cite `.siriIntent` as live signal anywhere.

   Do this instead: **add** a doc comment to `.siriIntent` (`PlaybackReason.swift:70` is a bare declaration — unlike its neighbors `.handoff` at `:71-74` and `.playAudioSchemaIntent` at `:86-88`, which carry one) saying plainly that it means a replayed `INPlayMediaIntent` continuation, that the path is currently dead, and that a zero reading proves nothing. Leave the `rawValue` alone — removing it is #830's call, not this PR's.
...
- `"media suggestion"` — PR 2's handler, i.e. a tile iOS dispatched to the SiriKit handler.
- `"PlayWXYCAudio intent"` — the audio schema, i.e. a tile iOS dispatched in place on iOS 27 (this series also carries genuine voice requests, which is why the baseline matters).

Without both, the observation cannot tell "tile fired and played in place" from "no tile at all."

**Do not read `"Siri intent"` as the foreground-launch signal — that was this plan's original mistake.** Code review of PR 2 established that `.siriIntent`'s only production call site, the third branch of `AppLifecycleModifier.handleUserActivity(_:)`, is **unreachable**: the method is registered for exactly two activity types (`org.wxyc.iphoneapp.play`, `NSUserActivityTypeBrowsingWeb`) and the two branches ahead of it consume both, while a real SiriKit replay arrives as activity type `INPlayMediaIntent`, which nothing registers for. The series is structurally empty, so a zero reading is not evidence of anything. Tracked as #830.

The practical consequence for this observation: **before PR 2 lands there is no series that can register a tile at all** on iOS 18.6–26, so on those versions the observation can only distinguish "tiles appear" from "no tiles" by eye, not in PostHog. That strengthens rather than weakens the case for PR 2 — it is the change that makes the measurement possible, not merely the fix the measurement evaluates.
