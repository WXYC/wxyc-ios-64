...
- **`/config` bootstrap (runtime).** `AppServices` re-exports the generated type via `@_exported import struct WXYCAPIModels.AppConfig` in `AppConfiguration.swift` (#915, the `Intents.swift` idiom) — replacing a hand-written twin whose drift nothing would have surfaced — the hazard class d970bd22a realized on other vendored DTOs, though the `/config` twin itself never drifted. Same flat-shape rationale as the album proxy. The re-export means `import AppServices` alone carries both the `AppConfig` name and, under Swift 6.2's member-import-visibility rule, its members — consumer files need no `WXYCAPIModels` import of their own, and no app target needs a `project.pbxproj` package dependency on it. The **scoped** form is load-bearing, not style: the generated package declares bare names that also exist in the `Playlist` package, so re-exporting or whole-importing the full module makes any unqualified use of a colliding name ambiguous. The collision that was observed breaking `NowPlayingWidget/Provider.swift` — `Playlist` as a type annotation with two candidates — is now historical: the 10.0.0 pin advance dropped `Models/Playlist.swift` upstream. `LiveFsEvent` still collides, declared by both this package and `Shared/Playlist/Sources/Playlist/V2/LiveFsEvent.swift`, so the rule keeps a live example and is not merely precautionary. Do not relax it on the strength of one name having gone away; the generated surface grows every pin advance, and the next collision arrives without warning. One semantic trade to know about: the generated struct has public `var` members and is Hashable where the deleted twin was `let`-immutable and Equatable, so config held outside the actor is immutable by convention, not by the compiler. `AppSecrets`'s runtime type stays hand-written **for now, but the generated twin now exists**: `/config/secrets` entered `api.yaml` in wxyc-shared commit `363718c`, and the 1.36.0 pin advance (#940) vendored `Models/AppSecrets.swift` into this tree. Nothing reads it yet — `fetchSecrets` still decodes into the hand-written struct in `AppConfiguration.swift`, so this is the same drift hazard #915 removed for `AppConfig`, deliberately deferred rather than fixed: the runtime swap is its own considered change, tracked as #945. Until that lands, treat the hand-written struct as authoritative for runtime and the generated one as unused. Note that `verify-api-types.sh` cannot catch drift between the two — it only proves the vendored tree matches the pinned contract, not that anything consumes it.
...
Exit code 0 means the committed tree matches a fresh regen from the pinned contract. This is enforced in CI: `.github/workflows/verify-api-types.yml` runs it on every PR touching `Shared/WXYCAPIModels/**` — or either of the two scripts that constitute the check (`scripts/verify-api-types.sh`, `scripts/regenerate-api-types.sh`), so an edit to the gate itself can't merge ungated — and again on every qualifying push to `master`, catching the case where two individually-green PRs merge into a drifted tree (a base-branch advance never re-fires a PR's own run). Runs on `ubuntu-latest` with no repository secrets (`wxyc-shared` is public) and a read-only token, and fails with the diff in the log on any drift. Still worth running locally after touching `contract-version.json`, to catch drift before pushing rather than in CI.
...
- `knownUnconsumedWireFields` — fields the generated type declares that the listener app deliberately does not decode yet (as of the 10.0.0 pin advance: `segue`, `rotation_bin` — DJ-tooling/rotation-scheduling signals the listener has no use for; `on_streaming`, `track_position` — plausible future features not yet built; `rotation_label`, `label_id`, `discogsUnavailable`, `discogsUnavailableNote` — arrived with 10.0.0 and parked pending #1091, which is where the adopt-or-acknowledge call for each gets made). The ledger's own doc comment carries the per-field reasoning; keep it there rather than duplicating it here.
