...
- **`/config` bootstrap (runtime).** `AppServices` re-exports the generated type under its historical name — `public typealias AppConfig = WXYCAPIModels.AppConfig` in `AppConfiguration.swift` (#915) — replacing a hand-written twin whose drift nothing would have surfaced — the hazard class d970bd22a realized on other vendored DTOs, though the `/config` twin itself never drifted. Same flat-shape rationale as the album proxy; every *type position* compiles unchanged through the typealias, but Swift 6.2's member-import-visibility rule required a one-line `import struct WXYCAPIModels.AppConfig` in each file that touches `AppConfig`'s members (eight files: the app/TV/watch entry points, the widget provider, the share extension, `DonateRowModel`, and both test files). The **scoped** form is load-bearing, not style: the generated package declares bare names that also exist in the `Playlist` package (`Playlist`, `LiveFsEvent`), so with both modules whole-imported any unqualified use of a colliding name is ambiguous (observed breaking `NowPlayingWidget/Provider.swift`, where `Playlist` as a type annotation had two candidates), and an `@_exported import` would spill the entire generated surface into every AppServices importer. `AppSecrets` stays hand-written: `/config/secrets` entered `api.yaml` in wxyc-shared commit `363718c` — one commit **after** this tree's pin (`da93c79`), still labeled 1.35.0 (the unmoved-version era wxyc-shared#347 ended) — so no generated twin exists in this tree; a future pin advance that vendors it should target ≥ 1.36.0 for the #348 wire-semantics doc comments.
