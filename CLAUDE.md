...
- **[`docs/configuration.md`](docs/configuration.md)** — `AppConfiguration` (PostHog/API/request-o-matic), Sentry dSYM upload (the build phase that fails an archive and warns on everything else, plus how it finds `sentry-cli` and its token), code-signing (team `92V374HC38`, automatic), extension targets (Share/Widget/CarPlay), widget refresh budget, App Store previews, minimum iOS version
